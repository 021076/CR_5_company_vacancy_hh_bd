        ...
        for employer in name_employers:
            ...
            try:
                if employers_response.status_code != 200:
                    raise Exception(f'Ошибка {employers_response.status_code}: {employers_response.json()}')
            except Exception:
                raise
            else:
    ...
    def get_vacancies(id_employer: str):
        ...
        try:
            if vacancies_response.status_code != 200:
                raise Exception(f'Ошибка {vacancies_response.status_code}: {vacancies_response.json()}')
        except Exception:
            raise
        else:
